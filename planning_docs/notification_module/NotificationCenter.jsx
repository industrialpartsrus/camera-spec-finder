...
// Add to your Listing Builder Pro, Photo Station, or Scanner app
// ============================================================
...
// ------------------------------------------------------------
const styles = {
  // Bell button (top-right corner of your app)
  bellButton: {
    position: 'relative',
    background: 'none',
    border: 'none',
...
// ------------------------------------------------------------
// Keyframe animation for urgent cards (inject once)
// ------------------------------------------------------------
...
// The bell icon with unread count badge
// ============================================================
...
// Full notification panel with setup, list, and management
// ============================================================
export default function NotificationCenter({ firebaseApp, userId, deviceName }) {
  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      // Test vibration so they know it works
  ...
  const handleDone = useCallback((notification) => {
    ...
    // Optionally call your API to mark the task complete
    if (notification.data?.sku) {
      fetch('/api/tasks/complete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sku: notification.data.sku }),
      }).catch(console.error);
    }
  }, []);

  // Snooze notification (remove from list, will re-appear in 5 min)
  const handleSnooze = useCallback((notification) => {
    setNotifications((prev) => prev.filter((n) => n !== notification));

    // Call API to re-send in 5 minutes
    if (notification.data?.sku) {
  ...
  return (
    <>
      {/* Bell icon — place this in your app's header/navbar */}
      ...
      {isOpen && (
        <>
          ...
          <div style={{ ...styles.panel, animation: 'slideIn 0.3s ease' }}>
            ...
            {/* Setup banner (if not yet enabled) */}
            {!isSetup && (

...
//
// All templates keep alt-text under 50 characters (SureDone field limit).
// ============================================================================
...
// Short view keywords for truncation fallback (Strategy 3)
const VIEW_KEYWORDS = {
  left: 'Left View',
  right: 'Right View',
  center: 'Front View',
 ...
 * @returns {string} Alt-text string, max 50 characters
 */
function generateAltText({ brand, partNumber, viewType, condition, productType, photoIndex }) {
  ...
  // === Smart 50-char limit for SureDone ===
  // Priority: brand + part > view > condition > productType
  if (altText.length > 50) {
    // Strategy 1: Drop condition
    let shorter = hasType ? template.withType : template.withoutType;
    shorter = shorter
      .replace(/\{brand\}/g, cleanBrand)
      .replace(/\{part\}/g, cleanPart)
      .replace(/\{type\}/g, cleanType)
    ...
    if (shorter.length <= 50) {
      ...
      // Strategy 2: Drop condition AND productType
      shorter = template.withoutType
        .replace(/\{brand\}/g, cleanBrand)
        .replace(/\{part\}/g, cleanPart)
        .replace(/\{type\}/g, '')
        .replace(/\{condition\}/g, '')
      ...
      if (shorter.length <= 50) {
        ...
        // Strategy 3: Minimal — just brand + part + view keyword
        const viewWord = VIEW_KEYWORDS[cleanView] || 'Photo';
        shorter = `${cleanBrand} ${cleanPart} ${viewWord}`.trim();

        if (shorter.length <= 50) {
          altText = shorter;
        } else {
          // Strategy 4: Brand + part only (truncate if still too long)
          altText = `${cleanBrand} ${cleanPart}`.substring(0, 50).trim();
        }
      }
    }
  }

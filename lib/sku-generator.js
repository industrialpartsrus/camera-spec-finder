 ...
 * @param {string} sku - SKU string like "AI0123" or "AI123"
 * @returns {number|null} - Numeric part (123) or null if invalid format
 */
function extractSkuNumber(sku) {
  if (!sku || typeof sku !== 'string') return null;
  // Use flexible regex to match any number of digits (AI172, AI0172, etc.)
  const match = sku.match(/^AI(\d+)$/i);
 ...
 * Uses the WORKING approach from suredone-create-listing.js
 * @returns {Promise<number>} - Highest SKU number found, or 0 if none
 */
async function getHighestSureDoneSku() {
  try {
    ...
    // Use /editor/items?search= endpoint (same as suredone-create-listing.js - WORKS!)
    const url = `https://api.suredone.com/v1/editor/items?search=sku:AI`;
    ...
    // Iterate through all keys, skipping SureDone metadata keys
    ...
    console.log(`SureDone highest SKU: AI${String(maxNum).padStart(4, '0')} (found ${skus.length} AI SKUs)`);
    return maxNum;
  } catch (error) {

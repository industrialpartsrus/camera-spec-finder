...
export default function ProListingBuilder() {
  ...
  const sendToSureDone = async (itemId) => {
    ...
    if (!item) {
      alert('âŒ Item not found in queue');
      return;
    }
    
    if (!item.title || !item.price) {
      alert('âš ï¸ Please fill in Title and Price before sending to SureDone');
      return;
    }
    ...
    try {
      ...
      const productData = {
        ...
        // Product category for BigCommerce mapping
        ...
        // eBay category - send both ID and name
        ebayCategoryId: item.ebayCategory?.id || '',
        ebayCategory: item.ebayCategory?.name || '',
        
        // Specifications as OBJECT (not array) for eBay item specifics mapping
      ...
      alert('âŒ Error sending to SureDone:\n\n' + error.message);
    }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b px-6 py-4">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold">Pro Listing Builder ðŸ”¥</h1>
            <p className="text-sm text-gray-600">Logged in: <span className="font-semibold">{userName}</span> â€¢ <span className="text-green-600">â— Live Sync</span></p>
          </div>
        </div>
            ...
            <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-sm p-4 lg:p-6">
              <div className="flex justify-between items-start mb-4 lg:mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Status: {selected.status} â€¢ By: {selected.createdBy}</p>
                </div>
              </div>
                ...
                <div className="space-y-4 lg:space-y-6">
                  ...
                  {selected.specifications && selected.specifications.length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <h4 className="font-semibold text-blue-900 mb-2">ðŸ“‹ Metadata Specifications (for search/filters)</h4>
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-1">
                        {selected.specifications.map((spec, idx) => (
                          <p key={idx} className="text-xs text-blue-800">â€¢ {spec}</p>
                        ))}
                      </div>
